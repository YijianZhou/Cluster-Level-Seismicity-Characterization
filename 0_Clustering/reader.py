...
def read_fault(ffault, lat_rng, lon_rng):
    ...
    with open(ffault, errors="replace") as f:
        for line in f:
            if not line.strip() or line[0] == '#':
                continue

            if line[0] == '>':
                if cur:                      # finish current segment
                    faults.append(np.asarray(cur, dtype=float))
                    cur = []
                continue

            lon, lat = map(float, line.split()[:2])
            if not (lon_rng[0] <= lon <= lon_rng[1] and lat_rng[0] <= lat <= lat_rng[1]):
                continue
            cur.append([lon, lat])
    ...
    return faults   # list of (Ni, 2) arrays
